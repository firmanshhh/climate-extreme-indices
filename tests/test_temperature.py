...
class TestTemperatureIndices:
    ...
    def test_idxtemp_invalid_input(self, sample_daily_temperature):
        ...
        # CARI INDEX PERTAMA YANG VALID (tanpa NaN di ketiga kolom)
        valid_mask = (
            ~df_invalid['TEMPERATURE_AVG_C'].isna() &
            ~df_invalid['TEMP_24H_TX_C'].isna() &
            ~df_invalid['TEMP_24H_TN_C'].isna()
        )
        valid_idx = df_invalid[valid_mask].index[0]  # Ambil index pertama yang valid
        
        # Force ketidakkonsistenan yang jelas pada index yang valid
        original_tmax = df_invalid.loc[valid_idx, 'TEMP_24H_TX_C']
        df_invalid.loc[valid_idx, 'TEMP_24H_TN_C'] = original_tmax + 10.0  # Tmin > Tmax
        
        # Pastikan juga TAVE tidak konsisten (TAVE harus di antara Tmin dan Tmax)
        df_invalid.loc[valid_idx, 'TEMPERATURE_AVG_C'] = original_tmax + 5.0  # TAVE > Tmax
